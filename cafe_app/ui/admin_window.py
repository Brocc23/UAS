...
class AdminWindow:
    def __init__(self, root, user):
        ...
        # WARNA & FONT
        self.colors = {
            "bg": "#f0f2f5", "card": "#ffffff", "primary": "#4a90e2", "primary_hover": "#357abd",
            "text_dark": "#2d3436", "text_grey": "#636e72", "danger": "#d63031", "danger_hover": "#c0392b",
            "success": "#2ecc71"
        }
        self.fonts = {
            "h1": ("Segoe UI", 24, "bold"), "h2": ("Segoe UI", 12), "body": ("Segoe UI", 10),
            "btn": ("Segoe UI", 10, "bold"), "table_head": ("Segoe UI", 10, "bold"),
    ...
    def setup_styles(self):
        ...
        style.configure("TNotebook.Tab", background=self.colors["bg"], foreground=self.colors["text_grey"],
                        padding=(20,10), font=self.fonts["btn"])
        ...
        style.configure("Treeview", background="white", fieldbackground="white", foreground=self.colors["text_dark"],
                        rowheight=35, font=self.fonts["table_body"], borderwidth=0)
        style.configure("Treeview.Heading", background=self.colors["bg"], foreground=self.colors["text_dark"],
                        font=self.fonts["table_head"], relief="flat")
    ...
    def create_custom_button(self, parent, text, command, bg_color, width=None):
        btn = tk.Button(parent, text=text, font=self.fonts["btn"], bg=bg_color, fg="white",
                        activebackground=bg_color, activeforeground="white", relief="flat", cursor="hand2",
                        pady=8, command=command)
    ...
    def build_ui(self):
        ...
        tk.Label(header, text=f"Halo, {self.user['username']} ({self.user['role']})", font=self.fonts["body"],
                 bg=self.colors["card"], fg=self.colors["text_grey"]).pack(side="right", pady=15)
    ...
    def build_menu_tab(self):
        ...
        left_col.pack(side="left", fill="y", padx=(0,20)); left_col.pack_propagate(False)
        card_form = self.create_card(left_col); card_form.pack(fill="x", pady=(0,20))
        ...
        btn_box = tk.Frame(card_form, bg=self.colors["card"]); btn_box.pack(fill="x", pady=20)
        ...
        # Preview
        card_preview = self.create_card(left_col); card_preview.pack(fill="x", expand=True)
        ...
        self.preview_label = tk.Label(card_preview, text="No Image", bg="#f0f2f5", height=10); self.preview_label.pack(fill="both", expand=True)
        # Table
        right_col = tk.Frame(main_layout, bg=self.colors["bg"]); right_col.pack(side="left", fill="both", expand=True)
        card_table = self.create_card(right_col); card_table.pack(fill="both", expand=True)
        ...
        table_frame = tk.Frame(card_table, bg="white"); table_frame.pack(fill="both", expand=True)
        ...
        for c in cols: self.menu_table.heading(c,text=c.upper()); self.menu_table.column(c,width=100,anchor="center")
        self.menu_table.bind("<ButtonRelease-1>", self.select_menu); self.load_menu()
    ...
    def _add_form_field(self, parent, label):
        ...
        container, entry = self.create_custom_entry(parent); container.pack(fill="x", pady=(0,10)); return entry

    # --- USER TAB ---
    def build_user_tab(self):
        main_layout = tk.Frame(self.tab_user, bg=self.colors["bg"]); main_layout.pack(fill="both", expand=True, padx=20, pady=20)
        card_form = self.create_card(main_layout); card_form.pack(fill="x", pady=(0,20))
        tk.Label(card_form, text="Kelola User", font=self.fonts["h2"], bg=self.colors["card"], fg=self.colors["primary"]).pack(anchor="w", pady=15)
        form_grid = tk.Frame(card_form, bg=self.colors["card"]); form_grid.pack(fill="x")
        ...
        action_box = tk.Frame(card_form, bg=self.colors["card"]); action_box.pack(fill="x", pady=20)
        ...
        card_table = self.create_card(main_layout); card_table.pack(fill="both", expand=True)
        cols = ("id","username","role"); self.user_table = ttk.Treeview(card_table, columns=cols, show="headings", style="Treeview")
        ...
        self.user_table.bind("<ButtonRelease-1>", self.select_user); self.load_users()

    def _add_grid_field(self,parent,text,r,c): tk.Label(parent,text=text,font=("Segoe UI",9,"bold"),bg=self.colors["card"],fg=self.colors["text_grey"]).grid(row=r,column=c,sticky="w")

    # --- FOTO ---
    def upload_foto(self):
        path = filedialog.askopenfilename(filetypes=[("Images","*.jpg;*.png")])
        if path: self.foto_menu_path=path; self.show_preview_foto(path)

    def show_preview_foto(self,path):
        self.preview_label.config(image="", text=""); self.menu_image=None
        if not path: self.preview_label.config(text="No Image Selected", fg=self.colors["text_grey"]); return
        if not os.path.exists(path): self.preview_label.config(text=f"File tidak ditemukan:\n{path}", fg=self.colors["danger"]); return
        try: img = Image.open(path); img.thumbnail((200,200)); self.menu_image = ImageTk.PhotoImage(img, master=self.window); self.preview_label.config(image=self.menu_image)
        except Exception as e: self.preview_label.config(text=f"Gagal memuat gambar:\n{str(e)}", fg=self.colors["danger"])

    # --- MENU LOGIC ---
    def add_menu(self):
        try: MenuModel().add_menu(self.nama_menu.get(), self.kategori_menu.get(), self.harga_menu.get(), self.stok_menu.get(), self.foto_menu_path); show_info("Menu berhasil ditambahkan."); self.load_menu(); self._clear_menu_form()
    ...
    def select_menu(self,event):
        ...
        if not s: return; d=self.menu_table.item(s)["values"]; self.selected_menu_id=d[0]
    ...
    def update_menu(self):
        ...
        MenuModel().update_menu(self.selected_menu_id,self.nama_menu.get(),self.kategori_menu.get(),self.harga_menu.get(),self.stok_menu.get(),self.foto_menu_path)
        show_info("Menu berhasil diupdate."); self.load_menu()

    def delete_menu(self):
        if self.user['role'] != 'admin': show_info("Hanya admin yang bisa menghapus menu."); return
        if not self.selected_menu_id: show_info("Pilih menu terlebih dahulu."); return
        if messagebox.askyesno("Konfirmasi","Yakin hapus menu ini?"): MenuModel().delete_menu(self.selected_menu_id); show_info("Menu berhasil dihapus."); self.load_menu(); self._clear_menu_form()

    def _clear_menu_form(self):
        self.nama_menu.delete(0,tk.END); self.kategori_menu.delete(0,tk.END); self.harga_menu.delete(0,tk.END); self.stok_menu.delete(0,tk.END)
        self.foto_menu_path=""; self.show_preview_foto("")

    # --- USER LOGIC ---
    def add_user(self):
        try: UserModel().register(self.user_username.get(),self.user_password.get(),self.user_role.get()); show_info("User berhasil ditambahkan."); self.load_users()
        except Exception as e: messagebox.showerror("Error",str(e))
    ...
    def select_user(self,event):
        ...
        if not s: return; d=self.user_table.item(s)["values"]; self.selected_user_id=d[0]
    ...
    def delete_user(self):
        if self.user['role'] != 'admin': show_info("Hanya admin yang bisa menghapus user."); return
        if not self.selected_user_id: show_info("Pilih user terlebih dahulu."); return
        if messagebox.askyesno("Konfirmasi","Yakin hapus user ini?"): UserModel().delete_user(self.selected_user_id); show_info("User berhasil dihapus."); self.load_users()
